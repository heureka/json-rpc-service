...
class RequestTest(unittest.TestCase):
    ...
    def test_parse_error(self):
        ...
        self.assertRaises(ParseError, request.id)

    def test_invalid_request(self):
        request = Request(parsed_request=[])
        self.assertRaisesRegex(InvalidRequest, 'Expected an object', request.id)
        self.assertIsNone(request.id())
        self.assertFalse(request.is_notification())

        request = Request(parsed_request={})
        self.assertRaisesRegex(InvalidRequest, 'Missing "jsonrpc"', request.id)
        self.assertIsNone(request.id())
        ...
        request = Request(parsed_request={"jsonrpc": "3.14"})
        self.assertRaisesRegex(InvalidRequest, 'version "2.0"', request.id)

        request = Request(parsed_request={"jsonrpc": "2.0"})
        self.assertRaisesRegex(InvalidRequest, 'Missing "method"', request.id)

        request = Request(parsed_request={"jsonrpc": "2.0", "method": 1})
        self.assertRaisesRegex(InvalidRequest, 'must be a string', request.id)

        request = Request(parsed_request={"jsonrpc": "2.0", "method": "foo"})
        self.assertEqual(request.method(), "foo")

        request = Request(parsed_request={"jsonrpc": "2.0", "method": "foo", "params": False})
        self.assertRaisesRegex(InvalidRequest, 'must be either object or array', request.id)
